                        ...
                        <div class="rounded-md border border-zinc-200 p-4">
                            ...
                            @if ($model->video?->source_url)
                                ...
                                <div class="mt-3 flex flex-wrap gap-3 text-sm font-semibold">
                                    ...
                                    @endif
                                    @if (! in_array($model->video->status, ['uploaded', 'processing'], true))
                                        <form method="POST" action="{{ route('admin.banners.reencode', $model) }}">
                                            @csrf
                                            <button type="submit" class="text-[#222052] hover:text-[#d9b44a]">Re-encode video</button>
                                        </form>
                                    @endif
                                </div>
                                @if ($model->video->error_message)
                                    <p class="mt-3 whitespace-pre-wrap break-words rounded-md bg-red-50 px-3 py-2 text-xs text-red-700">{{ $model->video->error_message }}</p>
                                @endif
                            @else
